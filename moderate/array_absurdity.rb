...
# Each line begins with a positive integer(N)
# i.e. the size of the array, then a semicolon followed by a comma separated list of positive numbers ranging from 0 to N-2, inclusive. i.e eg.
# 5;0,1,2,3,0
...
def find_duplicate(string)
  n, digits = string.split(";")
  
  digits.split(",").map(&:to_i).inject(:+) - ((n.to_i - 1) * (n.to_i - 2)) / 2
end

File.open(ARGV[0]).each_line do |line|
  puts find_duplicate(line.chomp)
end
